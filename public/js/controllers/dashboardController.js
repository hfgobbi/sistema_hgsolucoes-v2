...
app.controller('DashboardController', function($scope, $http) {
  ...
  $scope.carregarTendenciasComparativas = function() {
    ...
    $http.get('/api/dashboard/tendencias-comparativas?meses=6')
      ...
      .catch(function(error) {
        ...
        mostrarNotificacao('error', 'Erro ao carregar tendências comparativas');
      });
  };
  ...
  function renderizarGraficoTendencias() {
    ...
    try {
      ...
      } else if (window.graficoTendencias) {
        // Se existe mas não tem o método destroy, limpar a referência
        window.graficoTendencias = null;
      }
    } catch (e) {
      console.warn('Erro ao tentar destruir gráfico anterior:', e);
      // Continuar mesmo se houver erro na limpeza do gráfico anterior
    }
    
    // Verificar se há dados para o gráfico
    if (!$scope.dashboard.tendencias.dados || $scope.dashboard.tendencias.dados.length === 0) {
      console.log('Sem dados para renderizar o gráfico de tendências');
      return;
    }
    
    // Preparar dados para o gráfico
    const labels = $scope.dashboard.tendencias.dados.map(item => item.mes);
    const receitas = $scope.dashboard.tendencias.dados.map(item => item.receitas);
    const despesas = $scope.dashboard.tendencias.dados.map(item => item.despesas);
    const saldos = $scope.dashboard.tendencias.dados.map(item => item.saldo);
