module.exports = (app) => {
  ...
  controller.listar = async (req, res) => {
    try {
      ...
      if (mes && ano) {
        ...
        // Considerar tanto data quanto data_vencimento para a filtragem
        filtro[Op.or] = [
          {
            data: {
              [Op.gte]: dataInicio,
  ...
  controller.adicionar = async (req, res) => {
    try {
      ...
      // Validar parcelas se for movimentação parcelada
      if (tipo_frequencia === 'parcelada' && (!parcelas_total || parcelas_total < 2)) {
        return res.status(400).json({ message: 'Para movimentações parceladas, informe um número de parcelas válido (mínimo 2)' });
      }
      ...
      const novaMovimentacao = await Movimentacao.create({
        ...
        parcelas_total: tipo_frequencia === 'parcelada' ? parcelas_total : null,
        parcela_atual: tipo_frequencia === 'parcelada' ? 1 : null,
        usuario_id: usuarioId
      });
      
      // Se for parcelada, criar as demais parcelas
      console.log('Verificando criação de parcelas:', tipo_frequencia, parcelas_total);
      if (tipo_frequencia === 'parcelada' && parcelas_total > 1) {
        console.log('Criando parcelas para movimentação', novaMovimentacao.id);
        try {
          // Chamar o método do controller
          await controller.criarParcelas(novaMovimentacao, parcelas_total);
  ...
  // Função para criar as parcelas subsequentes de uma movimentação parcelada
  controller.criarParcelas = async function(movimentacaoPai, totalParcelas) {
    try {
      console.log('Iniciando criação de parcelas para movimentação:', movimentacaoPai.id);
      console.log('Detalhes da movimentação pai:', JSON.stringify(movimentacaoPai));
      
      // Obter os dados da movimentação pai
      const {
        descricao,
        valor,
        tipo,
      ...
      const dataVencimentoBase = movimentacaoPai.data_vencimento ? new Date(movimentacaoPai.data_vencimento) : new Date(data);
      console.log('Data vencimento base:', dataVencimentoBase);
      
      // Criar as parcelas restantes (2 até N)
      const parcelas = [];
      
      // Garantir que temos a data em formato string
      let dataOriginalStr;
      if (typeof data === 'string') {
        dataOriginalStr = data;
      } else if (data instanceof Date) {
        dataOriginalStr = data.toISOString();
      } else {
        dataOriginalStr = new Date(data).toISOString();
      }
      
      // Remover a parte de hora/minuto/segundo se existir
      const dataOriginalFormatada = dataOriginalStr.split('T')[0];
      console.log('Data original formatada:', dataOriginalFormatada);
      
      for (let i = 2; i <= totalParcelas; i++) {
        // Calcular a data de vencimento incrementando os meses
        const dataVencimento = new Date(dataVencimentoBase);
        dataVencimento.setMonth(dataVencimentoBase.getMonth() + (i - 1));
        const dataVencimentoFormatada = dataVencimento.toISOString().split('T')[0];
        
        console.log(`Parcela ${i}: Data=${dataOriginalFormatada}, Vencimento=${dataVencimentoFormatada}`);
        ...
        const parcela = {
          ...
          valor,
          tipo,
          data: dataOriginalFormatada,
          categoria_id,
          observacao,
          tipo_frequencia: 'parcelada',
          status_pagamento: 'pendente', // As parcelas futuras sempre começam como pendentes
          data_vencimento: dataVencimentoFormatada,
          data_pagamento: null, // parcelas futuras ainda não foram pagas
          parcelas_total: totalParcelas,
          parcela_atual: i,
          movimentacao_pai_id: movimentacaoPai.id,
          usuario_id
        };
      ...
      // Salvar todas as parcelas de uma vez
      if (parcelas.length > 0) {
        const parcelasCriadas = await Movimentacao.bulkCreate(parcelas);
        console.log(`Parcelas criadas com sucesso: ${parcelasCriadas.length}`);
      }
      
      // Atualizar a descrição da primeira parcela para incluir a informação da parcela
      await movimentacaoPai.update({
        descricao: `${descricao} (1/${totalParcelas})`,
        parcela_atual: 1 // Garantir que a parcela pai tenha o número correto
      });
      
      console.log(`Processo concluído. Criadas ${parcelas.length} parcelas adicionais para a movimentação ${movimentacaoPai.id}`);
