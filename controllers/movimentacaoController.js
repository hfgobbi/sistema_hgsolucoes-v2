module.exports = (app) => {
  ...
  const normalizarData = (dataString) => {
    ...
    // Se já estiver no formato ISO, apenas retornar a parte da data
    ...
    // Tentar converter para objeto Date e depois para string ISO
    try {
      ...
      if (tipo_frequencia === 'parcelada' && parcelas_total && parcelas_total > 1) {
        ...
        try {
          ...
          for (let i = 1; i < parcelas_total; i++) {
            ...
            const dataCompetenciaParcela = new Date(dataNormalizada);
            dataCompetenciaParcela.setMonth(dataCompetenciaParcela.getMonth() + i);
            const novaDataCompetencia = dataCompetenciaParcela.toISOString().split('T')[0];
            
            // Calcular a data de vencimento da próxima parcela
            const dataVencimentoParcela = new Date(dataVencimentoNormalizada);
            dataVencimentoParcela.setMonth(dataVencimentoParcela.getMonth() + i);
            const novaDataVencimento = dataVencimentoParcela.toISOString().split('T')[0];
